...
module.exports = {
    enroll:async(req, res) => {
        try {
            ...
            var s_id= mongosse.Types.ObjectId(req.params.s_id)
          ...
          if(result.enrollCode==e_code && result.allowEnroll)
          {
        ...
        {
          res.json({ success: false,message:"Wrong Code or Enrollment is closed." })
        }
        }
        catch (err) {
    ...
    attend:async(req, res) => {
      try {
            var c_id=mongosse.Types.ObjectId(req.body.cid)
            ...
            for(var k=0; k<s_id.length; k++){
                std = mongosse.Types.ObjectId(s_id[k])
    ...
    all: async (req, res) => {
     ...
     attend_notifier:async(req, res) => {
      try {
          var s_id= mongosse.Types.ObjectId(req.body.sids)
          var result=await student_model.find({"_id":s_id})
          console.log(result.name)
          console.log(result.regno)
          console.log("Presence of a student has been acknowledged.")
          res.status(200).json({ success: true, result:"Notified"})
      }
        catch (err) {
            res.status(501).json({success: false,message: "Notification failed."})
        }
    },

    student_home: async (req,res)=> {
      try{
        var s_id= mongosse.Types.ObjectId(req.params.id)
